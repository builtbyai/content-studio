...
```
app.example.com            Cloudflare Pages (React SPA)
      │  /api/*  (same-origin Worker route — no CORS)
      ▼
contentforge-api           Cloudflare Worker (Hono, 40+ routes)
  ├── Auth           D1 sessions, SHA-256(password || salt)
  ├── Content        articles / battlecards / sources (+ RSS cron ingest)
  ├── Media          R2 proxy, presigned + worker-proxied uploads
  ├── Generation     Workers AI image/chat via AI Gateway
  ├── Workflows      staged pipeline + D1 audit ledger
  ├── Scheduler      Postiz client, queue-driven publish
  ├── Sales          prospect / outreach / approval queue
  ├── Research       SEO + competitor intel (Vectorize)
  └── DO             ScheduleRoom — per-user SSE fan-out
```
...
Before deploying you'll need to create the Cloudflare resources (D1, R2, Queues, KV, Vectorize, AI Gateway) and fill the placeholder IDs in `worker/wrangler.toml`. Provider API keys are set as Worker secrets via `wrangler secret put` — never committed. The full production runbook is in [`docs/DEPLOY.md`](docs/DEPLOY.md).
